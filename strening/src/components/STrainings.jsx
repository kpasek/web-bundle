...
export class STrainings extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      scheduleTrainingId: urlParams.get("id"),
      loading: true,
    };
  }
  ...
  fetchData = async () => {
    const trainingRespone = await fetch(
      "https://api.kamilpasek.pl/api/schedule/trainings/" +
        this.state.scheduleTrainingId,
      {
        method: "GET",
        mode: "cors",
        cache: "no-cache",
    ...
    const sExercisesRespone = await fetch(
      "https://api.kamilpasek.pl/api/schedule/exercises/t/" +
        this.state.scheduleTrainingId,
      {
        method: "GET",
        mode: "cors",
    ...
    const exercisesRespone = await fetch(
      "https://api.kamilpasek.pl/api/exercises/part/" + parts[0].name,
      {
        method: "GET",
        mode: "cors",
        cache: "no-cache",
  ...
  handleSwapExercise = async (scheduleExerciseId) => {
    ...
    await fetch("https://api.kamilpasek.pl/api/schedule/exercises/swap", {
      method: "POST",
      mode: "cors",
      cache: "no-cache",
      credentials: "include",
  ...
  handleEdit = async (exerciseId, sExerciseId) => {
    ...
    const updateTask = fetch(
      "https://api.kamilpasek.pl/api/exercises/" + exerciseId,
      {
        method: "PUT",
        mode: "cors",
        cache: "no-cache",
    ...
    const changeOrderTask = fetch(
      "https://api.kamilpasek.pl/api/schedule/exercises/change-order",
      {
        method: "POST",
        mode: "cors",
        cache: "no-cache",
  ...
  handleAddExercise = async () => {
    ...
    const exerciseReponse = fetch(
      "https://api.kamilpasek.pl/api/schedule/exercises",
      {
        method: "POST",
        mode: "cors",
        cache: "no-cache",
  ...
  handlePartChange = async () => {
    ...
    const getExerciseSelectResponse = await fetch(
      "https://api.kamilpasek.pl/api/exercises/part/" + selectPart,
      {
        method: "GET",
        mode: "cors",
        cache: "no-cache",
  ...
  handleDelete = async (exerciseId) => {
    const deleteTask = fetch(
      "https://api.kamilpasek.pl/api/schedule/exercises/" + exerciseId,
      {
        method: "DELETE",
        mode: "cors",
        cache: "no-cache",
  ...
  handleDeleteTraining = async () => {
    ...
    await deleteTask;
    document.location.href = "/?page_id=33";
  };
  renderSelectPart() {
    return (
      <React.Fragment>
        ...
        <React.Fragment>
          <div className="">
            <div className="float-left mt-2 ml-2">
              <a
                href={"/?page_id=48&id=" + this.state.scheduleTrainingId}
